...
public abstract class DocumentSymbolProvider {
  protected static final Logger LOG = Logger.getInstance(MiscUtil.class);

  @NotNull
  public static DocumentSymbolProvider getDocumentSymbolProvider(@NotNull PsiFile file) {
    if (file instanceof KtFile) {
      return new KtDocumentSymbolProvider();
    } else if (file instanceof PyFile) {
      return new PyDocumentSymbolProvider();
    } else {
      return new JavaDocumentSymbolProvider();
    }
  }
  ...
  protected static String methodLabel(@NotNull PsiMethod method) {
    return method.getName() + Arrays.stream(method.getParameterList().getParameters())
        .map(DocumentSymbolProvider::methodParameterLabel)
