...
public class DefinitionCommandTest extends BasePlatformTestCase {
  ...
  public void testDefinitionJava() {
    ...
    checkDefinitions(path, answersJavaDefinition());
  }
  ...
  public void testDefinitionPython() {
    ...
    assertNotNull(virtualFile);
    final var path = LspPath.fromVirtualFile(virtualFile);
    checkDefinitions(path, answersPythonDefinition());
  }

  private void checkDefinitions(LspPath path, Map<Integer, LocationLink> answers) {
    final var file = MiscUtil.resolvePsiFile(getProject(), path);
    assertNotNull(file);
    final var doc = MiscUtil.getDocument(file);
    assertNotNull(doc);
    final int FILE_SIZE = 127;
  ...
  private Map<Integer, LocationLink> answersPythonDefinition() {
    ...
    answers.put(73, locationLink(firstUrl, a, a));
    answers.put(74, locationLink(firstUrl, a, range(8, 1, 8, 2)));
    answers.put(77, locationLink(secondUrl, A, range(8, 4, 8, 5)));
    answers.put(78, locationLink(secondUrl, A, range(8, 5, 8, 6)));
    answers.put(81, locationLink(firstUrl, aa, aa));
    answers.put(83, locationLink(firstUrl, aa, range(9, 2, 9, 3)));
    answers.put(86, locationLink(firstUrl, a, range(9, 5, 9, 6)));
    answers.put(87, locationLink(firstUrl, a, range(9, 6, 10, 0)));
    answers.put(88, locationLink(firstUrl, func, range(10, 0, 10, 4)));
    answers.put(92, locationLink(firstUrl, func, range(10, 4, 10, 5)));
    answers.put(96, locationLink(firstUrl, b1, b1));
    answers.put(98, locationLink(firstUrl, b1, range(12, 2, 12, 3)));
    ...
    answers.put(112, locationLink(firstUrl, b2, b2));
    answers.put(114, locationLink(firstUrl, b2, range(13, 2, 13, 3)));
